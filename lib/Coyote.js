...
function Coyote(amqpUri, type, queue, options) {
  ...
  switch (this.type) {
    case 'PUBLISH':
    case 'PUSH':
      this.mode = MODE_WRITE | MODE_PRODUCER;
      break;
    case 'SUBSCRIBE':
    case 'PULL':
      this.mode = MODE_READ;
    ...
    case 'WORKER':
      ...
      break;
    case 'REQUEST':
      this.mode = MODE_READ | MODE_WRITE | MODE_PRODUCER;
      break;
    case 'REPLY':
...
Coyote.prototype.write = function(response) {
  if (this.mode & MODE_WRITE) {
    this.emit('debug', 'Writing', response);
...
Coyote.prototype.cleanupConnection = function() {
  ...
  this.socket.on('close', function() {
    // Clean up after rabbit.js
    this.socket.ch.connection.close();
...
Coyote.prototype.shutdown = function() {
  this.transitionTo(this.STATE.SHUTDOWN);
};
